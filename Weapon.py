...
from space_objects import weapons
import vector_functions as vf
import global_parameters as paras


class WeaponSystem:
    def __init__(self, weapon_system_name: str):
        try:
            weapon_dict = weapons[weapon_system_name]
        except KeyError:
            print(f'No ship with name {weapon_system_name} in dict defined.')
            raise

        self.type = weapon_dict['type']
        self.cycle_time = weapon_dict['cycle_time']
        try:
            self.weapon_name = weapon_dict['weapon_name']
        except KeyError:
